...
Vamos verificar a qualidade dos ajustes utilizando a função `resid_plot`. Utilizamos `na.omit`
para ocultar as árvores não medidas:
```{r}
resid_plot(na.omit(dados), "TH", "CL","Henricksen")
```
...
Para estimar o volume, vamos utilizar uma equação ajustada previamente, com dados de cubagem:
```{r}
...
parcela, área da parcela, dap, altura,área total,volume com casca, altura dominante e idade.
Iremos obter com isso as variáveis dap, diâmetro quadrático, altura e altura dominante média por parcela. Além de número de indivíduos, volume total, e área basal por parcela e por hectare.
```{r}
tab_invt <- plot_summarise(df=dados,plot="PLOT",plot_area="PLOT_AREA",dbh="DBH",                           th="TH_EST",total_area="STRATA_AREA",vwb="VWB",dh="DH",age="AGE")
tab_invt   
```
