...
#' sprs(ex3_mfr, "VWB", "PLOT_AREA", "TOTAL_AREA", error = 20, pop = "fin")
#'
...
#' sprs(ex4_mfr, "VWB", "PLOT_AREA", "TOTAL_AREA", error = 20, pop = "fin")
#'
#' # The desired error was met
#' 
#' # area values can be numeric
#' sprs(ex4_mfr, "VWB", 3000, 46.8, error = 20, pop = "fin")
#' 
#' # Here we run a simple random sampling inventory for each forest subdivision, using the STRATA variable as a group variable:
#' sprs(ex2_mfr, "VWB", "PLOT_AREA", "STRATA_AREA",.groups = "STRATA" ,error = 20, pop = "fin")
#'
#' @author Sollano Rabelo Braga \email{sollanorb@@gmail.com}

sprs <- function(df,Yi, plot_area, total_area, age=NULL, .groups=NULL, alpha = 0.05, error = 10, dec_places=4, pop="inf",tidy=T){
  ...
  x_ <-df %>%
    ...
    dplyr::summarise(
      ...
      t_rec        = ifelse(pop=="inf", 
                            qt(alpha/2, df = ceiling( t^2 * VC^2 / error^2) - 1, lower.tail = FALSE)  ,
                            qt(alpha/2, df = ceiling( t^2 * VC^2 / ( error^2 +(t^2 * VC^2 / N) ) ) - 1, lower.tail = FALSE) ) ,
